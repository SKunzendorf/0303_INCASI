...
# load dataframes
## age
data_age <- read.csv(file = paste(path_variables, "INCASI_age.csv", sep=""), header=TRUE, sep = ';', dec = ",")  # hbp score df
...
flex_medsplit <- function(df, colname, list, plot = F) {
  ...
  # 4. plot medsplit
  if (plot == T) {
    plot_medsplit <- ggplot(data = medsplit, aes(x= vp, y = vec, colour = medsplit)) +
...
for (part in inc_list[1:length(inc_list)]) {  # loop over participants
  ...
  main_exg2_dtp[[part]] <- read.table(paste(path_data, part, '/crop/Kubios/', part, '_main_exg2.txt', sep = ''))
  # 
  # # Read time points from R peaks Kubios output
  # main_exg2_peaks_tmp <- NULL
  # main_exg2_peaks_tmp <- readMat(paste(path_data, part, '/crop/Kubios/', part, '_main_exg2_hrv.mat', sep = '')) # read Kubios matlab output into R
