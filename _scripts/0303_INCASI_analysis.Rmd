...
# change this param if you want to set the path manually
auto_set_wd <- menu(c("Yes", "No"), title="Shall I set WD automatically?", graphics = T)
...
if (auto_set_wd == 1){
  ...
  cat("WD is expected to be the cloned '0303_INCASI' folder on your machine in order to find dependencies.")
  own_wd <- readline('Please specify full path for WD: ')
  print(own_wd)
  tryCatch(setwd(own_wd),
           error = function(err) {
             setwd(script.dir)
             setwd('..')
             message(err)
             message('\nI try to set WD automatically.')
           })
}
