...
#### 2. Load dataframes for main analysis: 
...
# load defined cardiac phases (created in script `templates_t_q`)
...
#### 3. Load parameters for picture matching (EmoPicS)
* physical image paramenters (luminance, contrast, RGB, complexity)
* high-level parameters (number of people, group/social interaction, close-up, eye-contact)
```{r picture matching}
#----------------------------------------------------------------------------
# load dataframes
match_neu <- read.csv(file = paste(path_variables, "EmoPics_match_neu_o.csv", sep=""), header=TRUE, sep = ';', dec = ",")
match_neu$valence <- "neutral"

match_pos <- read.csv(file = paste(path_variables, "EmoPics_match_pos_o.csv", sep=""), header=TRUE, sep = ';', dec = ",")
match_pos$valence <- "positiv"

match_neg <- read.csv(file = paste(path_variables, "EmoPics_match_neg_o.csv", sep=""), header=TRUE, sep = ';', dec = ",")
match_neg$valence <- "negativ"

emopics_match <- rbind(match_pos, match_neu, match_neg)

emopics_match$valence <- factor(emopics_match$valence)
emopics_match[is.na(emopics_match)] <- 0 # replace NAs with 0

# save(emopics_match, file = paste(path_dataframes,"emopics_match.RData", sep="")) # save emopic picutre factors
#--------------------------------------------------------------------------
```


#### 4. Load dataframes for additional individual variables

* 1. Age
* 2. Blood pressure
* 3. Interoceptive Accuracy (IA)
* 4. Trait anxiety (STAI)
* 5. Heart Rate Variability (HRV: mean RR, rmssd)
...
# save(data_age, file = paste(path_dataframes,"data_age.RData", sep="")) # save stai

## 2. BP
data_bp <- read.csv(file = paste(path_variables, "BP_data.csv", sep=""), header=TRUE, sep = ';', dec = ",")
data_bp$vp <- as.factor(data_bp$vp)
# save(data_bp, file = paste(path_dataframes,"data_bp.RData", sep="")) # save blood pressure values

## 3. IA
load(paste(path_dataframes, "score.RData", sep="")) # hbp score df with IA scores
...
#### 5. Prepare supplementary analysis of picture ratings
...
* **diaspat**: remaining diastole (R-R - (pep + syspat + 50 ms)
...
for (part in inc_list[1:length(inc_list)]) {  # loop over participants
  ...
  for (iclick in (run+1):(run+length(log_encode$click1onset_ecgtime_s[log_encode$vp == part]))) {   # loop over 120 clicks for each vp
    ...
    log_encode$pep[iclick] <- sitvec["pep"] # pre-ejection period (one value per subject)
    ...
    # click in security intervals (pep, 50 ms)
    log_encode$click_pep[iclick] <- as.numeric((dif <= crop) | (dif > (Rtendx + diaspat))) #pep (i.e., falls either in crop, or in qonR)
...
log_encode$vp <- factor(log_encode$vp)
