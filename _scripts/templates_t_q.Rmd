...
# load(paste(path_dataframes,"main_exg2_dtp.RData",sep=""))  # list with ecg leads for each inc (if already created, see below)
...
### 1.A.1. Create list with single ecg leads
...
## 1.B. Create averaged Rst snippet and crop out template that contains t-wave (t_templave)
...
## 1.C. Trapez area approach (TRA): function to find end of twave
...
## 1.D.Apply function trapez_area to find twave end in t_template
...
for (isub in seq_along(qrst_list)) {
  ...
  #--------------------------------------------------------------------------
  ### 3. create q_templ
  q_templ[[isub]] <- qrs_template[[isub]][1:q] # template of qwave (q = substracted dpts from Rpeak = distance from beginning of template until Rpeak)
